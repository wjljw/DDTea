...
!(function () {
  var maskTpl = '<div class="ui-mask"></div>';
  function DialogAni(options) {
    this.options = $.extend(
      true,
      {
        $dialog: null,
    ...
    if (!$(this.options.$dialog)[0]) {
      throw new Error('没找到$dialog')
    }
    var $dialog = this.options.$dialog,
      $mask = this.$mask = $(maskTpl),
      style = {
        opacity: 0,
        position: 'fixed',
        left: '50%',
    ...
    open: function () {
      ...
      $dialog
        .animate(
          ...
          options.time,
          function () {
            $dialog.on('click', '.J-dialog-close', function () {
              self.close();
            });
  ...
  $.extend({
    ...
    dialog: function (options) {
      ...
      options.custom = options.custom && $(options.content)[0];

      var tpl = "<div class='ui-dialog'><div class='ui-dialog-header'><p class='ui-dialog-title'></p><img class='J-dialog-close ui-dialog-close' src='img/close.png' /></div><div class='ui-dialog-body'></div><div class='ui-dialog-footerWrap'></div></div>",
        $dialog = options.custom && $(options.content).show()
          ?
          $(options.content)
          :
          $(tpl);
      ...
      if (!options.custom) {
        ...
        $dialog
          ...
          .html(options.title)
        options.onInit($dialog);
      }

      if (!options.custom && options.width) {
        $dialog.width(options.width);
      }

      var ani = new DialogAni({
        $dialog: $dialog,
        onClose: options.onClose,
        onOpen: options.onOpen
      })
      ani.open();
      $dialog.dialogHide = function () {
        ani.close.call(ani);
      }
      return $dialog;
    },
    alert: function ( msg ) {
      ...
      options.onInit = function ($dialog) {
        $dialog
          .find('.ui-dialog-footerWrap')
          .append('<div class="ui-dialog-footer"><div class="btn btn-size1 btn-dark J-dialog-close">确定</div></div>')
      }
      $.dialog(options);
    },
    confirm: function ( msg ) {
      ...
      options.onInit = function ($dialog) {
        $dialog
          .find('.ui-dialog-footerWrap')
          .append('<div class="ui-dialog-footer clearfix"><div class="w_50p f_l"><div class="btn btn-size1 btn-danger J-dialog-close J-dialog-cancel">取消</div></div><div class="w_50p f_l"><div class="btn btn-size1 btn-dark J-dialog-close J-dialog-confirm">确定</div></div></div>')
      }
      options.onOpen = function ($dialog) {
        $dialog
          .on('click', '.J-dialog-cancel', function () {
            d.reject();
          })
          .on('click', '.J-dialog-confirm', function () {
            d.resolve();
          });
      }
      $.dialog(options);
    ...
    tips: function (msg) {
      var $dialog = $('<div class="ui-tipsWrap" style="position:fixed;opacity:0"><div class="ui-tips">' + msg + '</div></div>');
      $('body').append($dialog);
      $dialog
        .animate(
          {
...
function Banner(options) {
  ...
  if (options.$wrap instanceof jQuery && options.$overflow instanceof jQuery && options.$items instanceof jQuery) {
      this.options = options;
      this.index = 0;
      this.max = options.$items.length - 1;
      this.addEvent();
      this.play();
  } else {
      throw new Error('options.$wrap、options.$overflow、options.$items必须为jquery对象')
  }
}
Banner.prototype = {
  interval: null,
  ...
  move: function (index) {
      ...
      this.options.onmove(index);
      $overflow
          .animate(
              {
                  marginLeft: parseInt($overflow.css('marginLeft')) - this.options.$items.eq(index).position().left
              },
              this.options.duration,
              function () {
                  self.options.onmoved(index);
              }
          );
  },
  addEvent: function () {
